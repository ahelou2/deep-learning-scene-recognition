function [err pstates states gendata] = c_unsuperv_offline_eval(X, model, gibbs_iter, direction)
...
%                 multinomial_exp_sample(pstates{k}.hid_top(:,:,i), model{k}.C2D, model{k}.N_H2D) ;
    ...
    if model{1}.type == 'BB'
        ...
        data = pgendata > rand(numcases,model{1}.N_V2D(1)*model{1}.N_V2D(2));   
