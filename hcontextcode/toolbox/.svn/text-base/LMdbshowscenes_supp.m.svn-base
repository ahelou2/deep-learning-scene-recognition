function LMdbshowscenes_supp(D, HOMEIMAGES, subimages, showanno)
%
        ...
        for x = 0:Nx-1
            ...
            if nargin>1
                ...
                if showanno
                    ...
                    [h,class] = LMplot_supp(annotation, img); % show image and polygons
                else
                    imagesc(img)
                    axis('off'); axis('equal'); axis('tight')
                end
            else
                [h,class] = LMplot_supp(D(i).annotation); % show image and polygons
                axis('ij')
            end
            drawnow
        end
