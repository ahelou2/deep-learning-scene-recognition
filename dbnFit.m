function [model, error]= dbnFit(X, numhid, y, varargin)
...
H=length(numhid);
model=cell(H,1);
% First phase, unsupervised
if H>=2
    
    %train the first RBM on data
    if(numopts>=1)
        [model{1}, error{1}]= rbm(X, numhid(1),varargin{1});
    else
        [model{1}, error{1}]= rbm(X, numhid(1));
    end
    
    %train all other RBM's on top of each other
    for i=2:H-1
        if(numopts>=i)
            [model{i}, error{i}] =rbm(model{i-1}.top, numhid(i), varargin{i});
        else
            [model{i}, error{i}] =rbm(model{i-1}.top, numhid(i));
        end
    ...
    if(numopts>=H)
        [model{H}, error{H}]= rbm_fit(model{H-1}.top, numhid(end), y, varargin{H});
    else
        [model{H}, error{H}]= rbm_fit(model{H-1}.top, numhid(end), y);
    end
else
    
    %numhid is only a single layer... but we should work anyway
    if (numopts>=1)
        [model{1}, error{1}]= rbm_fit(X, numhid(1), y, varargin{1});
    else
        [model{1}, error{1}]= rbm_fit(X, numhid(1), y);
    end
end    
